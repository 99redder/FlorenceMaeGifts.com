...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname !== '/api/stripe-webhook') {
      ...
      const isAccountsRead = ['/api/accounts/list','/api/accounts/summary','/api/accounts/journal','/api/accounts/statements','/api/accounts/invoices','/api/accounts/invoices/detail','/api/accounts/quotes','/api/accounts/quotes/detail','/api/admin/notes'].includes(url.pathname) && request.method === 'GET';
      const isAccountsWrite = ['/api/accounts/journal','/api/accounts/rebuild-auto-journal','/api/accounts/year-close','/api/accounts/invoices','/api/accounts/invoices/update','/api/accounts/invoices/status','/api/accounts/invoices/payment','/api/accounts/invoices/payment-link','/api/accounts/invoices/send','/api/accounts/invoices/delete','/api/accounts/quotes','/api/accounts/quotes/update','/api/accounts/quotes/delete','/api/accounts/quotes/send','/api/accounts/quotes/convert','/api/admin/notes'].includes(url.pathname) && request.method === 'POST';
...
async function handleAdminNotesList(request, env, corsHeaders, url) {
  ...
  if (!auth.ok) return auth.res;
  const scope = (url.searchParams.get('scope') || 'general').toString().trim().toLowerCase();
  const yearRaw = (url.searchParams.get('year') || '').toString().trim();
  const year = /^\d{4}$/.test(yearRaw) ? Number(yearRaw) : null;
  const rows = scope === 'year'
    ? await env.DB.prepare(`SELECT id, scope_type, note_year, note_text, created_at, updated_at FROM admin_notes WHERE scope_type = 'year' AND note_year = ?1 ORDER BY created_at DESC, id DESC`).bind(year).all()
    : await env.DB.prepare(`SELECT id, scope_type, note_year, note_text, created_at, updated_at FROM admin_notes WHERE scope_type = 'general' ORDER BY created_at DESC, id DESC`).all();
  return json({ ok: true, notes: rows.results || [] }, 200, corsHeaders);
}
...
async function handleAdminNotesCreate(request, env, corsHeaders, url) {
  ...
  let data;
  try { data = await request.json(); } catch { return json({ ok: false, error: 'Invalid JSON' }, 400, corsHeaders); }
  const scope = (data.scope || 'general').toString().trim().toLowerCase();
  const noteText = (data.noteText || '').toString().trim();
  const year = /^\d{4}$/.test(String(data.year || '').trim()) ? Number(data.year) : null;
  if (!['general','year'].includes(scope)) return json({ ok: false, error: 'Invalid note scope' }, 400, corsHeaders);
  if (!noteText) return json({ ok: false, error: 'Note text is required' }, 400, corsHeaders);
  if (scope === 'year' && !year) return json({ ok: false, error: 'Valid note year is required' }, 400, corsHeaders);
  const r = await env.DB.prepare(`INSERT INTO admin_notes (scope_type, note_year, note_text) VALUES (?1, ?2, ?3)`).bind(scope, scope === 'year' ? year : null, noteText).run();
